...
public abstract class MyGSYBaseVideoPlayer extends MyGSYVideoControlView {
    ...
    //旋转工具类
    protected MyOrientationUtils mOrientationUtils;
    ...
    public MyGSYBaseVideoPlayer(@NonNull Context context) {
        super(context);
    }

    public MyGSYBaseVideoPlayer(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public MyGSYBaseVideoPlayer(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public MyGSYBaseVideoPlayer(Context context, Boolean fullFlag) {
     ...
     * @param to
     */
    protected void cloneParams(MyGSYBaseVideoPlayer from, MyGSYBaseVideoPlayer to) {
     ...
     * 全屏的暂停返回的时候返回页面不黑色
     */
    private void pauseFullBackCoverLogic(MyGSYBaseVideoPlayer gsyVideoPlayer) {
        //如果是暂停状态
     ...
     * 全屏
     */
    protected void resolveFullVideoShow(Context context, final MyGSYBaseVideoPlayer gsyVideoPlayer, final FrameLayout frameLayout) {
        ...
        mOrientationUtils = new MyOrientationUtils((Activity) context, gsyVideoPlayer, getOrientationOption());
        mOrientationUtils.setEnable(isRotateViewAuto());
     ...
     * 恢复
     */
    protected void resolveNormalVideoShow(View oldF, ViewGroup vp, MyGSYVideoPlayer gsyVideoPlayer) {
    ...
    protected void backToNormal() {
        ...
        final View oldF = vp.findViewById(getFullId());
        final MyGSYVideoPlayer gsyVideoPlayer;
        if (oldF != null) {
            gsyVideoPlayer = (MyGSYVideoPlayer) oldF;
            //如果暂停了
    ...
    protected Runnable mCheckoutTask = new Runnable() {
        @Override
        public void run() {
            MyGSYVideoPlayer gsyVideoPlayer = getFullWindowPlayer();
            if (gsyVideoPlayer != null
    ...
    public MyGSYBaseVideoPlayer startWindowFullscreen(final Context context, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            MyGSYBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
        } catch (Exception e) {
        ...
        try {
            //通过被重载的不同构造器来选择
            Constructor<MyGSYBaseVideoPlayer> constructor;
            final MyGSYBaseVideoPlayer gsyVideoPlayer;
            if (!hadNewConstructor) {
                constructor = (Constructor<MyGSYBaseVideoPlayer>) MyGSYBaseVideoPlayer.this.getClass().getConstructor(Context.class);
                gsyVideoPlayer = constructor.newInstance(mContext);
            } else {
                constructor = (Constructor<MyGSYBaseVideoPlayer>) MyGSYBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
     ...
     */
    @SuppressWarnings("ResourceType, unchecked")
    public MyGSYBaseVideoPlayer showSmallVideo(Point size, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            Constructor<MyGSYBaseVideoPlayer> constructor = (Constructor<MyGSYBaseVideoPlayer>) MyGSYBaseVideoPlayer.this.getClass().getConstructor(Context.class);
            MyGSYBaseVideoPlayer gsyVideoPlayer = constructor.newInstance(getActivityContext());
            ...
            gsyVideoPlayer.setVideoAllCallBack(mVideoAllCallBack);
            gsyVideoPlayer.setSmallVideoTextureView(new MySmallVideoTouch(gsyVideoPlayer, marginLeft, marginTop));
    ...
    public void hideSmallVideo() {
        final ViewGroup vp = getViewGroup();
        MyGSYVideoPlayer gsyVideoPlayer = (MyGSYVideoPlayer) vp.findViewById(getSmallId());
        removeVideo(vp, getSmallId());
        mCurrentState = getGSYVideoManager().getLastState();
        if (gsyVideoPlayer != null) {
            cloneParams(gsyVideoPlayer, this);
    ...
    public MyGSYVideoPlayer getFullWindowPlayer() {
        ...
        MyGSYVideoPlayer gsyVideoPlayer = null;
        if (full != null) {
            gsyVideoPlayer = (MyGSYVideoPlayer) full;
        }
        return gsyVideoPlayer;
    }
    ...
    public MyGSYVideoPlayer getSmallWindowPlayer() {
        ...
        MyGSYVideoPlayer gsyVideoPlayer = null;
        if (small != null) {
            gsyVideoPlayer = (MyGSYVideoPlayer) small;
        }
        return gsyVideoPlayer;
    }
     ...
     */
    protected void checkAutoFullWithSizeAndAdaptation(final MyGSYBaseVideoPlayer gsyVideoPlayer) {
        if (gsyVideoPlayer != null) {
